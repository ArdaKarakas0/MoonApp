    ...
    return (
        <Card title="Your Reflections">
            <textarea
                ...
                rows={5}
                className="w-full bg-celestial-blue/50 border border-starlight-silver/20 rounded-lg px-4 py-2 text-white placeholder-starlight-silver/50 focus:ring-2 focus:ring-moonbeam-gold focus:border-moonbeam-gold focus:outline-none transition-all"
            />
            <div className="text-right -mb-2">
                <button 
                    onClick={handleSaveJournal}
                    disabled={saveStatus === 'saving' || !isUnsaved}
                    className="bg-moonbeam-gold/80 text-celestial-blue font-bold py-2 px-4 rounded-lg hover:bg-amber-300 transition-colors duration-300 shadow-moonbeam-gold/20 text-sm disabled:bg-starlight-silver/20 disabled:text-starlight-silver/50 disabled:cursor-not-allowed"
                >
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-2xl mx-auto animate-fade-in space-y-6">
        <header className="text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-serif font-bold text-white">{readingContent.moonPhaseHeading.title}</h1>
          <p className="mt-2 text-starlight-silver/80">{readingContent.moonPhaseHeading.description}</p>
        </header>
        ...
        <div className="text-center bg-celestial-blue/40 py-4 px-6 rounded-lg border border-starlight-silver/10">
            ...
            <h2 className="text-2xl font-serif font-semibold text-moonbeam-gold tracking-wide">{readingContent.lunarAlignment}</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card title="Opportunity Window">
                <p className="text-2xl font-bold text-white text-center">{readingContent.opportunityWindow}</p>
            </Card>
        </div>

        <Card title="Symbol of the Day" className="text-center">
            <p className="text-xl font-serif font-semibold text-white">{readingContent.lunarSymbol.name}</p>
            <p className="italic text-starlight-silver/80">{readingContent.lunarSymbol.meaning}</p>
        </Card>
